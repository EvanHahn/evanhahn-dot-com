...
author: Evan Hahn
layout: post
path: /choose-by-weight/
---

_In short: Sum up the weights. Pick a random number between 0 and the total weight. Iterate. [Click here for a JavaScript solution.](https://gist.github.com/1135646)_
...
Imagine that you're standing at the left end of the strip with a stone. You throw the stone randomly so that it lands _somewhere_ on the strip. If your throw is completely random, it's most likely to land on the 4, right? It might not -- it might land on any of the others -- but it's "weighted" to land on the 4.
